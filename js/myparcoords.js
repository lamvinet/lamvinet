// Modified by Xin Rong for lamvi. 2017.2.24.
d3.parcoords = function(config) {
  var __ = {
    ...
    rotateLabels: false,
    midOffset: { axesIdx: 5, hiddenAxes: [4,5], offset: 500},
    binaryAxes: ['cbow'],
    forceRescaleY: false,
  };
...
pc.autoscale = function() {
    ...
    "number": function(k) {
      ...
      if (extent[0] === extent[1]) {
        return d3.scale.ordinal()
          .domain([extent[0]])
          .rangePoints(getRange());
      } else if (__.binaryAxes.indexOf(k) > -1) {
        return d3.scale.linear()
          .domain([-0.3, 1.3])
          .range(getRange());
      } else {
        /*
        if (k == 'size') extent=[20,110];
        if (k == 'window') extent = [1,11];
        if (k == 'lockf') extent = [-1.1,1.1];
        if (k == 'alpha') extent = [-0.05, 0.55];
        if (k == 'retro') extent = [-1.1, 2.1];
        if (k == 'triples') extent = [extent[0]-0.1, extent[1]+0.1];
        if (k == 'accuracy') extent = [extent[0]-0.1, extent[1]+0.1];
        if (k == 'synonyms') extent = [extent[0]-0.1, extent[1]+0.1];
        if (k == 'antonyms') extent = [extent[0]-0.1, extent[1]+0.1];
        */
        
        if (k == 'idx1') extent = [extent[0]-0.4, extent[1]+0.3];
        if (k == 'idx2') extent = [extent[0]-0.4, extent[1]+0.3];
        
        param_options.forEach(function(n) {
          if (k == n.name) {
            extent = [n.values[0] - 0.1, n.values[n.values.length - 1] + 0.1];
          }
        });

        if (k in OUTPUT_RANGES) {
          extent = [OUTPUT_RANGES[k][0] - 0.1, OUTPUT_RANGES[k][1] + 0.1];
        }
  ...
  d3.keys(__.dimensions).forEach(function(k) {
    if (!__.dimensions[k].yscale || __.forceRescaleY){
      __.dimensions[k].yscale = defaultScales[__.dimensions[k].type](k);
    }
  });

  if (__.forceRescaleY) {
    __.forceRescaleY = false;
  }

  // xscale
  xscale.rangePoints([0, w() - __.midOffset.offset], 1);
...
pc.applyDimensionDefaults = function(dims) {	
  ...
  dims.forEach(function(k) {
    ...
    //Set up defaults (these are consumed at pc.applyAxisConfig)
    ...
    newDims[k].tickValues = __.binaryAxes.indexOf(k) > -1 ? [0, 1] : null;
    newDims[k].tickFormat = __.binaryAxes.indexOf(k) > -1 ? d3.format('d') : null;

    param_options.forEach(function(n) {
      if (k == n.name) {
      	newDims[k].tickValues = $.extend(true, [], n.values);
        newDims[k].tickValues.sort();
      	newDims[k].ticks = n.values.length;
      }
    });

    /*
    if (k == 'size') newDims[k].tickValues = [30,50,100];
    else if (k == 'window') newDims[k].tickValues = [2,4,6,8,10];
    else if (k == 'lockf') newDims[k].tickValues = [-1,0,0.3,0.7,1];
    else if (k == 'alpha') newDims[k].tickValues = [0.01, 0.05, 0.1, 0.2, 0.5];
    else if (k == 'retro') newDims[k].tickValues = [-1,0.1,0.5,1,2];
    if (k == 'alpha') newDims[k].tickFormat = d3.format('.2f');
    */
...
pc.forceRescaleY = function(){
  __.forceRescaleY = true;
};
...
pc.createAxes = function() {
  ...
  g = pc.svg.selectAll(".dimension")
    ...
    .enter().append("svg:g")
      ...
      .attr("transform", function(d, i) {
        var offset = xscale(d);
        if (i >= __.midOffset.axesIdx ) {
          offset += __.midOffset.offset;
        }
        if (i == __.midOffset.axesIdx - 1) {
          pc.midOffsetBegin = offset;
        }
        if (i == __.midOffset.axesIdx) {
          pc.midOffsetEnd = offset;
        }
  ...
  g.append("svg:g")
      ...
      .each(function(d, i) {
        ...
        // this is also copied to updateAxes
        if (__.midOffset.hiddenAxes.indexOf(i) > -1) {
          axisElement.selectAll("path").style("visibility", "hidden");
          axisElement.selectAll("line").style("visibility", "hidden");
          axisElement.selectAll(".tick").style("visibility", "hidden");
        }
    ...
    .append("svg:text")
      .attr({
        "text-anchor": "middle",
        "y": 0,
        "transform": "translate(0,-12.5) rotate(" + __.dimensionTitleRotation + ")",
        "x": 0,
        "class": "label"
      })
      .text(function(d, i) { return __.midOffset.hiddenAxes.indexOf(i) > -1 ? '' : dimensionLabels(d)})
  ...
  g_data.enter().append("svg:g")
    ...
    .append("svg:g")
      ...
      .each(function(d,i) {
        ...
        // copied from createAxes (see also "Update" below)
        if (__.midOffset.hiddenAxes.indexOf(i) > -1) {
          axisElement.selectAll("path").style("visibility", "hidden");
          axisElement.selectAll("line").style("visibility", "hidden");
          axisElement.selectAll(".tick").style("visibility", "hidden");
  ...
  g_data.select(".axis")
    .transition()
      ...
      .each(function(d,i) {
        ...
        // copied from createAxes (see also "Enter" above)
        if (__.midOffset.hiddenAxes.indexOf(i) > -1) {
          axisElement.selectAll("path").style("visibility", "hidden");
          axisElement.selectAll("line").style("visibility", "hidden");
          axisElement.selectAll(".tick").style("visibility", "hidden");
  ...
  g.style("cursor", "move")
    .call(d3.behavior.drag()
      .on("dragstart", function(d) {
        if (__.dimensions[d].index < __.dimensions['idx1'].index) {  // LHS
          dragging[d] = this.__origin__ = xscale(d);
        } else if (__.dimensions[d].index > __.dimensions['idx2'].index) {  // RHS
          dragging[d] = this.__origin__ = xscale(d) + __.midOffset.offset;
        }
      })
      .on("drag", function(d) {
        if (__.dimensions[d].index < __.dimensions['idx1'].index) {  // LHS
          dragging[d] = Math.min(xscale('idx1'), Math.max(0, this.__origin__ += d3.event.dx));
          pc.sortDimensions();
          xscale.domain(pc.getOrderedDimensionKeys());
          pc.render();
          ...
          dragging[d] = Math.min(w(), Math.max(position('idx2')+1, this.__origin__ += d3.event.dx));
          pc.sortDimensions();
          xscale.domain(pc.getOrderedDimensionKeys());
          pc.render();
          g.attr("transform", function(d) {
      ...
      .on("dragend", function(d) {
        ...
        delete dragging[d];
        d3.select(this).transition().attr("transform", "translate(" + position(d) + ")");
        pc.render();
      }));
